...
public class IVARenderer : MonoBehaviour
{
    ...
    [Header("Colour (continuous parameters)")]
    ...
    [Header("Auto-fit to a host object (optional, off by default)")]
    // OPTIONAL host integration, OFF by default.
    //
    // When on (Play mode only), the avatar auto-scales UNIFORMLY to the largest size that
    // still fits inside an invisible reference frame, whose height is capped to the height of
    // a named object already in your scene (`hostObjectName`). Use this when the avatar must
    // sit in a fixed slot - a dashboard, a panel, a screen - and keep filling that slot as its
    // shape changes. The frame is a pure scale guide: a GameObject named `sizeFrameName` is
    // created next to the avatar with its renderer disabled, so it is never visible.
    //
    // If no object called `hostObjectName` exists, auto-fit simply does nothing and the
    // avatar keeps its authored Transform scale. Leave this off for standalone use.
    public bool  autoFitToSizeFrame  = false;
    public string hostObjectName = ""; // name of the object whose world-bounds height caps the frame
    public string sizeFrameName       = "size_restriction";
    [Range(0.05f, 1f)] public float frameHeightFraction = 0.8f; // frame height as a fraction of the host object's height (headroom so glow/ears/wave stay inside)
    ...
    float ResolveFrameWorldHeight()
    {
        ...
        if (_dashRendererCache == null && Time.unscaledTime >= _dashNextSearchTime)
        {
            ...
            if (string.IsNullOrEmpty(hostObjectName)) return -1f;
            var dash = GameObject.Find(hostObjectName);
